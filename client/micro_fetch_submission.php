...
$apiKey = "d37e51d6aad09b119b74247149f5fadb"; // Your JotForm API Key
$formId = "250771650764461"; // Your JotForm Form ID

// Calculate the threshold date (15 days ago)
$thresholdDate = date("Y-m-d", strtotime("-15 days"));

// **Delete Old Submissions from JotForm (ignores lab_id, only checks date)**
...
if (!empty($data['content'])) {
    foreach ($data['content'] as $submission) {
        ...
        $submissionDate = date("Y-m-d", strtotime($submission['created_at']));
        if ($submissionDate < $thresholdDate) {
...
// **Fetch Latest Submission from JotForm**
...
if (!empty($data['content'])) {
    ...
    $quantity = 0;
    $labId = 3; // Since this is for Microbiological Testing
