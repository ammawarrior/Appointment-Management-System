        ...
        setTimeout(() => {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick: function (info) {
                    ...
                    const sameCategorySubmissions = submissionsOnDate.filter(s => s.category === category);
                    const totalQuantitySameCategory = sameCategorySubmissions.reduce((sum, s) => sum + parseInt(s.quantity), 0);

                    if (lab_id == 1) {
                        if (category === "Renewal" && day !== 6) {
                            swal("Invalid Date!", "Renewals are allowed only on Saturdays.", "error");
                            return;
                        }

                        if (category === "New" && ![3, 4, 5, 6].includes(day)) {
                            swal("Invalid Date!", "New requests are allowed only on Wed, Thu, Fri, or Sat.", "error");
                            return;
                        }

                        if (request_type === "In-House" && day !== 6) {
                            swal("Invalid Date!", "In-House requests are only allowed on Saturdays.", "error");
                            return;
                        }

                        if (request_type === "On-Site" && ![3, 4, 5].includes(day)) {
                            swal("Invalid Date!", "On-Site requests are allowed only on Wed, Thu, or Fri.", "error");
                            return;
                        }

                        if ([3, 4, 5].includes(day) && totalQuantitySameCategory >= 1) {
                            swal("Date Full!", "Only 1 request allowed on Wed–Fri.", "error");
                            return;
                        }

                        if (day === 6 && totalQuantitySameCategory >= 4) {
                    ...
                    if (lab_id == 2) {
                        ...
                        if (totalQuantitySameCategory + parseInt(quantity) > 10) {
                ...
                dayCellDidMount: function (arg) {
                    ...
                    if (lab_id == 1) {
                        if (category === "Renewal" && day === 6 && totalQuantitySameCategory < 4) isValid = true;
                        if (category === "New") {
                            if ([3, 4, 5].includes(day) && totalQuantitySameCategory < 1) isValid = true;
                            if (day === 6 && totalQuantitySameCategory < 4) isValid = true;
                        }
                        if (request_type === "In-House" && day === 6 && totalQuantitySameCategory < 4) isValid = true;
                        if (request_type === "On-Site" && [3, 4, 5].includes(day) && totalQuantitySameCategory < 1) isValid = true;
                    } else if (lab_id == 2) {
                        if ((["Feed - No Waste", "Food - Perishable"].includes(category) && [1, 2].includes(day) && totalQuantitySameCategory < 10) ||
                            (category === "Waste Water - Treated" && day === 3 && totalQuantitySameCategory < 10)) isValid = true;
                    } else if (lab_id == 3) {
                        if ((["Food - Perishable", "Swab"].includes(category) && [1, 2].includes(day) && totalQuantitySameCategory < 10) ||
                            (category === "Waste Water - Treated" && day === 3 && totalQuantitySameCategory < 10)) isValid = true;
                    } else if (lab_id == 4) {
                        if ((analysis === "Shelf Life Evaluation" || analysis === "Sensory Evaluation") && [1, 2].includes(day) && submissionsOnDate.length < 2) isValid = true;
                    } else {
                        isValid = true;
                    }
                    ...
                    const quantityLeft = (lab_id == 4) ? (2 - submissionsOnDate.length)
                        : (lab_id == 2 || lab_id == 3) ? (10 - totalQuantitySameCategory)
                        : (lab_id == 1 && [3, 4, 5].includes(day)) ? (1 - totalQuantitySameCategory)
                        : (lab_id == 1 && day === 6) ? (4 - totalQuantitySameCategory) : null;
